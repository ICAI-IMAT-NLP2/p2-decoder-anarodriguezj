...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        ...
        # Optional mask
        if mask is not None:
            # Assign VERY negative values to the masked positions so that after
            # applying softmax, their probability becomes 0.
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, hidden_state, mask=None):
        ...
        head_outs = [head(hidden_state, mask) for head in self.heads] # pass mask to each head
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        att_output = self.self_attention(norm1, mask)
        x = x + att_output
        
        norm2 = self.layer_norm_2(x)
        feedforward_output = self.feed_forward(norm2)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = pos_ids.unsqueeze(0) >= pos_ids.unsqueeze(1)
